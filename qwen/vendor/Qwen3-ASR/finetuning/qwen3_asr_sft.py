...
def parse_args():
    ...
    p.add_argument("--load_best_model_at_end", type=int, choices=(0, 1), default=0)
...
def main():
    ...
    # Load each split independently. A metadata field can be entirely null in
    # one mixture (for example, U provenance when U=0) but populated in the
    # frozen validation set. Loading both files in one call makes Datasets
    # infer a shared Arrow schema and incorrectly fixes that field to `null`.
    raw_ds = DatasetDict(
        {
            "train": load_dataset(
                "json", data_files=args_cli.train_file, split="train"
            )
        }
    )
    if args_cli.eval_file:
        raw_ds["validation"] = load_dataset(
            "json", data_files=args_cli.eval_file, split="train"
        )
    ...
    training_args = TrainingArguments(
        ...
        load_best_model_at_end=(args_cli.load_best_model_at_end == 1),
        metric_for_best_model="eval_loss" if args_cli.load_best_model_at_end == 1 else None,
        greater_is_better=False if args_cli.load_best_model_at_end == 1 else None,
